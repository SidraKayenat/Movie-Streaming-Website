...
export async function getMoreInfo(req, res) {
    ...
    try {
        ...
        if (type === 'tv') {
            const reviewsUrl = `https://api.themoviedb.org/3/tv/${id}/reviews?language=en-US`;
            const reviewsResponse = await fetchFromTMDB(reviewsUrl);
            response.reviews = reviewsResponse.results || []; // Add reviews to the response

            // Fetch cast (starring) for the TV show
            const castUrl = `https://api.themoviedb.org/3/tv/${id}/credits?language=en-US`;
            const castResponse = await fetchFromTMDB(castUrl);
            response.cast = castResponse.cast || []; // Add cast to the response

            // Fetch creators of the TV show
            const creatorsUrl = `https://api.themoviedb.org/3/tv/${id}/created_by?language=en-US`;
        ...
        if (type === 'person') {
            const personDetailsUrl = `https://api.themoviedb.org/3/person/${id}/combined_credits?language=en-US`;
